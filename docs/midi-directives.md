...
| `%%MIDI channel <n>` | `<midi-channel>` | Set alongside a program (merged into the voice's instrument) or standalone. A standalone channel still emits an instrument (the name falls back to the part name — see below). |
...
voice's instrument lands in its own `<score-part>`. The `<score-instrument>`
`<instrument-name>` is the General MIDI name when the voice has a program,
otherwise the **part name** — never abc2xml's literal `no name` filler. Argument
parsing is lenient (matching abc2midi): a trailing `% comment` and any
non-numeric trailing words (e.g. `program 74 recorder`) are ignored; the leading
integer run is taken; out-of-range values are skipped.
